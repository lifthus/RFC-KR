...
만약 클라이언트가 표현의 부분 카피를 가지고 있고 전체 표현의 최신 카피를 가지길 바란다면, 조건부 GET과 함께 Range 헤더 필드를 사용할 수 있다(If-Unmodified-Since와 If-Match 둘 중 하나 혹은 둘 다를 사용하며.) 그러나, 만약 표현이 수정됐기 때문에 사전조건이 실패한다면, 클라이언트는 전체 현재 상태를 얻기 위해 두번째 요청을 만들어야 한다.

"If-Range" 헤더 필드는 클라이트가 두번째 요청을 "short-circuit"할 수 있도록 한다. 비공식적으로, 그 의미는 다음과 같다: 만약 표현이 변경되지 않았다면, 나에게 내가 Range에서 요청하고 있는 부분(들)을 보내라; 그렇지 않으면, 전체 표현을 보내라.
